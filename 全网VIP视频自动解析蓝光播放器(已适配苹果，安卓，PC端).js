// ==UserScript==
// @name         全网VIP视频自动解析蓝光播放器(已适配苹果，安卓，PC端)
// @namespace    https://www.tampermonkey.net/
// @namespace    https://greasyfork.org/zh-CN/scripts/440369/
// @version      1.1.9.6
// @license      AGPL-3.0
// @description  无需跳转新网址，打开官网直接看，蓝光 4k 无广告 随机去水印。支持：腾讯，爱奇艺，优酷，哔哩哔哩，咪咕，乐视，搜狐，芒果，西瓜，PPTV，1905电影网，华数。支持解析失败自动切换推荐解析源。适配各种浏览器。新增去广告功能去除浏览器搜索广告。
// @description   新增加视频嗅探，m3u8视频下载 视频下载转mp4（该功能目前只试配PC端）。
// @author       充钱是不可能充钱的
// @match        *://v.qq.com/*
// @match        *://m.v.qq.com/*
// @match        *://www.iqiyi.com/*
    ...
    if (window === top) {
        ...
        */
        settings.NoAD解析 = {    // TODO by 17kyun.com/api.php?url=    // TODO by tv.hzwdd.cn
                    ...
                    "PPJ蓝光解析": "https://bf.ppjbk.cn/?url=",    // TODO 腾讯 (芒果)
                    "PPJ蓝光弹幕解析": "https://jx.ppjbk.cn/?url=",    // TODO 腾讯 (芒果) (B站)
                    "PPJ（B站接线）": "https://bf.ppjbk.cn/bf/?url=",    // TODO 腾讯 (芒果)
                    ...
                    "天翼解析": "https://jsap.attakids.com/?url=" || "https://www.qianyicp.com/vip/vip_g.php?url=",    // _4K解析: "",    // TODO 腾讯 爱奇艺 (优酷) 咪咕 乐视 (芒果) 西瓜 PPTV
                    "人人迷解析": "https://jx.blbo.cc:4433/?url=",   // TODO 腾讯 [爱奇艺] 优酷 (乐视) 芒果 (1905电影网) [华数]
                    "FosiPlayer解析": "https://play.banyung.vip:7799/?url=",   // TODO 腾讯 [爱奇艺] 优酷 (乐视) 芒果 (1905电影网) [华数]
                    "Mao全网解析": "https://www.91jxs.com/jiexi/?url=",  // TODO 腾讯 [爱奇艺] 优酷 (乐视) 芒果 (1905电影网) [华数]
                    "虾米解析": "https://jx.xmflv.com/?url=" || "https://nbjx.vip/index.php?url=",    // TODO (土豆) (咪咕) 搜狐 (芒果)    // (Parwix解析: "https://jx.parwix.com:4433/player/analysis.php?v=" || "https://vip.parwix.com:4433/player/?url=" || )    // TODO (西瓜) (B站) (搜狐) (芒果)
                    "夜幕解析": "https://www.yemu.xyz/?url="    // TODO 土豆 华数 芒果:可能有广告    // by www.yunboys.cn
                };
                settings.AD解析 = {    // TODO (有赌博广告，请勿相信，这么简单的骗术不会有人上当吧)
                    ...
                    "Parwix解析": "https://jx.parwix.com:4433/player/?url="    // TODO 无水印(但不稳定): 腾讯 爱奇艺 优酷 乐视 [芒果] (PPTV) (华数)
                };
        start();
    ...
    if (window === top) {
        ...
        if (!detectMobile()) {
            if (location.host.indexOf("v.qq.com") !== -1) {
                readyPlayerBox("已进入腾讯视频", ["#mask_layer", ".mod_vip_popup", "#mask_layer"], [settings.NoAD解析["PPJ蓝光解析"],settings.NoAD解析["PPJ蓝光弹幕解析"],settings.NoAD解析["天翼解析"],settings.NoAD解析["人人迷解析"],settings.NoAD解析["ok解析"],settings.NoAD解析["Mao全网解析"],settings.NoAD解析["夜幕解析"],settings.NoAD解析["FosiPlayer解析"]], "div#mod_player", null);
            } else if (location.host.indexOf("iqiyi.com") !== -1) {
                readyPlayerBox("已进入爱奇艺", ["#playerPopup", "div[class^=qy-header-login-pop]"], [settings.NoAD解析["PPJ蓝光解析"],settings.NoAD解析["PPJ蓝光弹幕解析"],settings.NoAD解析["天翼解析"],settings.NoAD解析["人人迷解析"],settings.NoAD解析["ok解析"],settings.NoAD解析["Mao全网解析"],settings.NoAD解析["夜幕解析"],settings.NoAD解析["FosiPlayer解析"]],"iqpdiv.iqp-player[data-player-hook$=er]", null);
            } else if (location.host.indexOf("youku.com") !== -1) {
                readyPlayerBox("已进入优酷视频", ["#iframaWrapper"], [settings.NoAD解析["PPJ蓝光解析"],settings.NoAD解析["PPJ蓝光弹幕解析"],settings.NoAD解析["天翼解析"],settings.NoAD解析["人人迷解析"],settings.NoAD解析["ok解析"],settings.NoAD解析["Mao全网解析"],settings.NoAD解析["夜幕解析"],settings.NoAD解析["FosiPlayer解析"]],"div#player", null);
                ...
                doElement("svg[aria-hidden=true]", () => readyPlayerBox("已进入哔哩哔哩", null, [settings.NoAD解析["PPJ（B站接线）"],settings.NoAD解析["夜幕解析"], settings.NoAD解析["人人迷解析"],settings.NoAD解析["ok解析"],settings.NoAD解析["FosiPlayer解析"]], "div.bpx-player-video-area,div.mask-container", null));    // TODO || document.getElementById("bilibiliPlayer") || document.getElementById("live-player-ctnr")
                ...
                readyPlayerBox("已进入乐视TV", null, [settings.NoAD解析["PPJ蓝光解析"]], "#le_playbox", null);
                ...
                readyPlayerBox("已进入芒果TV", null, [settings.NoAD解析["PPJ蓝光解析"], settings.NoAD解析["PPJ蓝光弹幕解析"], settings.NoAD解析["天翼解析"], settings.NoAD解析["人人迷解析"], settings.NoAD解析["ok解析"],settings.NoAD解析["夜幕解析"],settings.NoAD解析["FosiPlayer解析"]], "#mgtv-player-wrap", null);
                ...
                readyPlayerBox("已进入PPTV", null, [settings.NoAD解析["PPJ蓝光解析"]], "div.w-video", null);
                ...
                readyPlayerBox("已进入华数TV", null, [settings.NoAD解析["PPJ蓝光解析"]], "div#pcplayer", null);
            }
        } else {
            if (location.host.indexOf("v.qq.com") !== -1) {
                ...
                readyPlayerBox("已进入爱奇艺", ["div.m-iqyGuide-layer", "a[down-app-android-url]", "[name=m-extendBar]", "[class*=ChannelHomeBanner]", "section.m-hotWords-bottom"],[settings.NoAD解析["PPJ蓝光解析"],settings.NoAD解析["PPJ蓝光弹幕解析"],settings.NoAD解析["天翼解析"],settings.NoAD解析["ok解析"],settings.NoAD解析["Mao全网解析"],settings.NoAD解析["夜幕解析"],settings.NoAD解析["FosiPlayer解析"]], "section.m-video-player", null);
            } else if (location.host.indexOf("youku.com") !== -1) {
                readyPlayerBox("已进入优酷视频", ["#iframaWrapper", ".ad-banner-wrapper", ".h5-detail-guide,.h5-detail-vip-guide", ".brief-btm"], [], "#player", null);
            } else if (location.host.indexOf("bilibili.com") !== -1) {
                readyPlayerBox("已进入哔哩哔哩", ["div.fe-ui-open-app-btn,div.recom-wrapper,open-app-btn", "[class*=openapp]"], [settings.NoAD解析["PPJ蓝光解析"], settings.NoAD解析["PPJ蓝光弹幕解析"], settings.NoAD解析["人人迷解析"], , settings.NoAD解析["ok解析"],settings.NoAD解析["Mao全网解析"],settings.NoAD解析["夜幕解析"],settings.NoAD解析["FosiPlayer解析"]], "div#app.main-container div.player-wrapper", null, href => href.replace("m.bilibili.com", "www.bilibili.com"));
            } else if (location.host.indexOf("miguvideo.com") !== -1) {
                ...
                readyPlayerBox("已进入乐视TV", ["a.leapp_btn", "div.full_gdt_bits[id^=full][data-url]", "[class*=Daoliu],[class*=daoliu],[class*=game]", "div.m-start", "[class*=icon_user]"], [settings.NoAD解析["云解析"],settings.NoAD解析["FosiPlayer解析"]], "div.column.play", null);
            } else if (location.host.indexOf("m.tv.sohu.com") !== -1) {
                readyPlayerBox("已进入搜狐视频", ["div[class^=banner]", "div.js-oper-pos", "div[id^=ad],div[id^=ad] *", "[id*=login],[class*=login]", "[class$=-app]", "div.app-vbox.ph-vbox,div.app-vbox.app-guess-vbox", "div.twinfo_iconwrap", "div[class$=banner],div[id$=banner]"], [settings.NoAD解析["夜幕解析"],settings.NoAD解析["FosiPlayer解析"]], "#player,#sohuplayer,.player-view", null, async href => {
                ...
                readyPlayerBox("已进入芒果TV", ["div.adFixedContain,div.ad-banner,div.m-list-graphicxcy.fstp-mark", "div[class^=mg-app],div#comment-id.video-comment div.ft,div.bd.clearfix,div.v-follower-info", "div.ht.mgui-btn.mgui-btn-nowelt", "div.personal", "div[data-v-41c9a64e]"], [settings.NoAD解析["PPJ蓝光解析"], settings.NoAD解析["PPJ蓝光弹幕解析"], settings.NoAD解析["天翼解析"], settings.NoAD解析["人人迷解析"], settings.NoAD解析["ok解析"],settings.NoAD解析["Mao全网解析"],settings.NoAD解析["夜幕解析"],settings.NoAD解析["FosiPlayer解析"]],"div.video-poster,div.video-area", null);
            } else if (location.host.indexOf("ixigua.com") !== -1) {
                ...
                readyPlayerBox("已进入PPTV", ["[data-darkreader-inline-bgimage][data-darkreader-inline-bgcolor]", "div[class^=pp-m-diversion]", "section#ppmob-detail-picswiper", "section.layout.layout_ads", "div.foot_app", "div[modulename=导流位]", "a[class*=user]", "div.mod_video_info div.video_func"], [settings.NoAD解析["PPJ蓝光解析"],settings.NoAD解析["FosiPlayer解析"]], "section.pp-details-video", null, href => href.replace("m.pptv.com", "v.pptv.com"));
            } else if (location.host.indexOf("vip.1905.com") !== -1) {
                (movie_info => {
                ...
                readyPlayerBox("已进入华数TV", ["div.ws_poster", "div.appdown,div.player_menu_con", "div#play_and_info_fix_adv"], [settings.NoAD解析["PPJ蓝光解析"]], "div#player,div#pop", null);
            }
        }
        ...
        function readyPlayerBox(Tip, displayNones, srcs, cssString, doFunction, doHref = null) {
            ...
            onLocationChange(() => location.reload());    // TODO 如果网页rul变了就刷新页面
                ...
                settings.DIY_iframeFunction = () => {
                    if (settings.parseDB.DIY解析栏) {
                        if (!DIY_iframe || DIY_iframe.constructor.name !== "HTMLDivElement") {
                            ...
                            DIY_iframe_text.onkeydown = DIY_iframe_select.onkeydown = event => {
                                if (event.key === "Enter") {
                                    DIY_iframe_src();
                                }
                            }
...
(function() {
	...
	// 解析接口（可多个）
	const parseInterfaces =["https://bf.ppjbk.cn/?url=","https://jx.ppjbk.cn/?url=","https://play.banyung.vip:7799/?url=","https://api.okjx.cc:3389/jx.php?url=","https://nbjx.vip/index.php?url=","https://jx.quanmingjiexi.com/?url=","https://jx.aidouer.net/?url=","https://jsap.attakids.com/?url=","https://jx.blbo.cc:4433/?url=","https://www.91jxs.com/jiexi/?url=","https://jx.xmflv.com/?url=","https://www.yemu.xyz/?url=","https://z1.m1907.cn?jx=","https://jx.parwix.com:4433/player/?url=",];
	/*
	 * 非自定义区域
	 * 以下代码勿动
	 * 以下代码勿动
